class ChangeChatConversationAccountIdsToTimestampIds < ActiveRecord::Migration[6.0]
  def up
    # Prepare the function we will use to generate IDs.
    Rake::Task['db:define_timestamp_id'].execute

    # Set up the chat_conversation_accounts.id column to use our timestamp-based IDs.
    ActiveRecord::Base.connection.execute(<<~SQL)
      ALTER TABLE chat_conversation_accounts
      ALTER COLUMN id
      SET DEFAULT timestamp_id('chat_conversation_accounts')
    SQL
  ...
  def down
    ...
    ActiveRecord::Base.connection.execute(<<~SQL)
      LOCK chat_conversation_accounts;
      SELECT setval('chat_conversation_accounts_id_seq', (SELECT MAX(id) FROM chat_conversation_accounts));
      ALTER TABLE chat_conversation_accounts
        ALTER COLUMN id
        SET DEFAULT nextval('chat_conversation_accounts_id_seq');
    SQL
  end
end
