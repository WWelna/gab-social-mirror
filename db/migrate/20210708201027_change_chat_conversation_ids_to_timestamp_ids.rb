class ChangeChatConversationIdsToTimestampIds < ActiveRecord::Migration[6.0]
  def up
    # Prepare the function we will use to generate IDs.
    Rake::Task['db:define_timestamp_id'].execute

    # Set up the chat_conversations.id column to use our timestamp-based IDs.
    ActiveRecord::Base.connection.execute(<<~SQL)
      ALTER TABLE chat_conversations
      ALTER COLUMN id
      SET DEFAULT timestamp_id('chat_conversations')
    SQL
