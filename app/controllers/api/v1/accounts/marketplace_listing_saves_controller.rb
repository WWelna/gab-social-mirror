...
class Api::V1::Accounts::MarketplaceListingSavesController < Api::BaseController
  ...
  def next_path
    if records_continue?
      api_v1_account_marketplace_listing_saves_url pagination_params(max_id: pagination_max_id)
    end
  end

  def prev_path
    unless @saves.empty?
      api_v1_account_marketplace_listing_saves_url pagination_params(min_id: pagination_since_id)
    end
  end
